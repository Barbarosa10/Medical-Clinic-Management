...
const Patients = () => {
    ...
    const saveDoctor = async(row) => {
        let url = "";
        console.log(getCookie('user').role);

        if(getCookie('user').role == 'ADMIN'){
            url = `${host}/api/medical_office/physicians/${row.id}`
        }
        ...
        if(url != ""){
            try{
                ...
                const response = await axios.put(url,
                    {
                        firstname: row.firstname, lastname: row.lastname, email: row.email, phone: row.phone, specialization: row.specialization, id_user: row.id_user
                    },
                    {
                    headers: {
                        "Authorization": "Bearer " + getJWT('user')
    ...
    const handleEdit = (index) => {
      ...
      const handleDelete = async (id) => {
        try {
            ...
            const response = await axios.delete(`${host}/api/medical_office/physicians/${id}`, {
                headers: {
                    "Authorization": "Bearer " + getJWT('user')
                }
            });
                    ...
                    <table style={{ margin: 'auto' }}>
                        ...
                        <tbody>
                            {data.map((row, index) => (
                                ...
                                {(getCookie('user').role == "ADMIN") &&<td>
                                    {editableRowIndex === row.id_user ? (
                                        <select id="dropdown" value={row.specialization} onChange={(e) => handleInputChange(row.id_user, 'specialization', e.target.value)}
                                        style={{ width: '80%', margin: 'auto', backgroundColor: index % 2 === 0 ? '#45a049' : 'white' }}>
